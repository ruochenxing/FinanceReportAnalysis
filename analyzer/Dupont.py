...
class Dupont:
    ...
    def analysis(self):
        ...
        netIncomeE = (self.index["净利率"][0] * self.index["资产周转率"][-1] * self.index["权益乘数"][-1]) - (
                self.index["净利率"][-1] * self.index["资产周转率"][-1] * self.index["权益乘数"][-1])
        tofTotalE = (self.index["净利率"][0] * self.index["资产周转率"][0] * self.index["权益乘数"][-1]) - (
                self.index["净利率"][0] * self.index["资产周转率"][-1] * self.index["权益乘数"][-1])
        leverageE = (self.index["净利率"][0] * self.index["资产周转率"][0] * self.index["权益乘数"][0]) - (
                self.index["净利率"][0] * self.index["资产周转率"][0] * self.index["权益乘数"][-1])
        region = (self.index["净利率"][-1] * self.index["资产周转率"][-1] * self.index["权益乘数"][-1])
        now = (self.index["净利率"][0] * self.index["资产周转率"][0] * self.index["权益乘数"][0])
        if now - region >= 0:
            print("正增长：ROE 从 {}% 改变至 {}%".format(round(region * 100, 2), round(now * 100, 2)))
            if netIncomeE == max(netIncomeE, tofTotalE, leverageE):
                ...
                invDay = (self.index["存货在库天数"][0] - self.index["存货在库天数"][-1]) / self.index["存货在库天数"][
                    -1]

                # self.index["应收帐款周转率"]
                rbleDay = (self.index["应收款项周转天数"][0] - self.index["应收款项周转天数"][-1]) / \
                          self.index["应收款项周转天数"][-1]
                # self.index["应收款项周转天数"]
                if invDay == max(invDay, rbleDay):
                    print("主要原因为【存货周转率】的改变。 存货在库天数从 {} 天 变为 {} 天".format(
                        self.index["存货在库天数"][-1],
                        self.index["存货在库天数"][0]))
                else:
                    print("主要原因为【应收账款周转率】的改变。 应收款项周转天数从 {} 天 变为 {} 天".format(
                        self.index["应收款项周转天数"][-1],
            ...
            print("负增长：ROE 从 {}% 改变至 {}%".format(round(region * 100, 2), round(now * 100, 2)))
