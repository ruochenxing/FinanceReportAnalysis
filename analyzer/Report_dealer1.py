...
# 分析某个财报，取上一年的当时财报 进行同比分析
class Report_dealer1:
    def __init__(self, scode='603993', report_date='2022-03-31'):
        self.scode = scode
        self.report_date = report_date
        self.year = report_date[0:4]
        self.info = {}
        self.info0 = {}
        # 获取某一年的所有财报
        origin_data = self.getData(self.scode, self.year)
        print("获取到以下财报")
        for k, v in origin_data.items():
            if "-" in k:
                print(k)
        # 取某一个财报
        data = origin_data[report_date]
        # 合并资产负债表，利润表，现金流量表
        for item in data:
            self.info.update(data[item])
        # 取上一年的
        pre_year = str(int(self.year) - 1)
        pre_origin_data = self.getData(self.scode, pre_year)
        print("获取到以下财报")
        for k, v in pre_origin_data.items():
            if "-" in k:
                print(k)

        data1 = pre_origin_data[pre_year + report_date[4:10]]
        for item in data1:
            self.info0.update(data1[item])
        # 取最近一年的财报
        # self.last12 = self.getData_n12(self.scode)
    ...
    def getData(self, scode, year):
        ...
        data = mgtable.find({"code": str(scode), "year": int(year)}, {"_id": 0})
        return data[0]
    ...
    def cash_flow_st(self):
        ...
        cash_flow_dict["+ 投资活动现金流量 (from 资产负债表左)"] = float(
            self.info["投资活动产生的现金流量净额"])  # 投资活动产生的现金流量净额
        # + 融资活动现金流量 (from 资产负债表右)
        cash_flow_dict["+ 融资活动现金流量 (from 资产负债表右)"] = float(
            self.info["筹资活动产生的现金流量净额"])  # 筹资活动产生的现金流量净额
        # 期末现金
        cash_flow_dict["期末现金"] = float(self.info["六、期末现金及现金等价物余额"])
        # 自由现金流（FCF）=经营活动产生的现金流量净额-购建固定资产、无形资产和其他长期资产支付的现金
        cash_flow_dict["自由现金流（FCF）"] = self.info["经营活动产生现金流量净额"] - self.info[
    ...
    def five_ratio(self):
        ...
        tmp = (float(info["在建工程"]) + float(info["固定资产净额"]) + float(info["工程物资"]))
        if tmp > 0:
            dict0["长期资金占重资产比率(%)"] = (float(info["所有者权益(或股东权益)合计"]) + float(
                info["非流动负债合计"])) / tmp
        ...
        '''偿债能力'''
        dict1 = {"kind": "财务结构"}
        dict1["流动比率(%)"] = float(info["流动资产合计"]) / float(
            info["流动负债合计"])  # 流动比率(Current Ratio) = 流动资产总额(Total Assets) / 流动负债总额(Current Liabilities)
        if info['应收票据及应收账款'] == 0:
            receivable = (info["应收账款"]) + (info["应收票据"])  # 应收账款+应收票据
        else:
            receivable = info["应收票据及应收账款"]
        dict1["速动比率(%)"] = (float(info["货币资金"]) + float(info["交易性金融资产"]) + receivable + float(
            info["应收利息"]) + float(info["应收股利"]) + float(info["其他应收款"])) / float(info[
                                                                                                 "流动负债合计"])  # 速动比率(%) = (货币资金 + 交易性金融资产 + 应收票据及应收账款 + 合同资产 + 应收股利 + 应收利息 + 其他应收款) / 流动负债(Total Liabilities)

        '''缺利息费用，暂时用财务费用'''
        if info["财务费用"] > 0:
            dict1["利息保障倍数"] = (float(info["四、利润总额"]) - float(info["财务费用"])) / float(
                info["财务费用"])  # 利息保障倍数(Interest Coverage) = 息税前利润(EBITDA) / 利息费用(Interest Expense)

        '''运营能力'''
        dict2 = {"kind": "财务结构"}

        dict2["应收款项周转率(次/年)"] = float(info["营业收入"]) / float(
            (info0["应收账款"] + info["应收账款"]) / 2)  # 应收款项周转率(次/年) = 营业收入/平均应收账款
        dict2["应收款项周转天数(天)"] = 365 / (
                float(info["营业收入"]) / float((info0["应收账款"] + info["应收账款"]) / 2))
        ...
        if info0["固定资产净额"] + info["固定资产净额"] > 0:
            dict2["固定资产周转率(次/年)"] = float(info["营业收入"]) / float(
                (info0["固定资产净额"] + info["固定资产净额"]) / 2)
        # dict2["完整生意周期(天)"] =
        # 销售成本÷平均应付账款
        ...
        dict3["ROE=净资产收益率(%)"] = float(info["五、净利润"]) / float(
            (info0["所有者权益(或股东权益)合计"] + info["所有者权益(或股东权益)合计"]) / 2)
        ...
        dict3["营业费用率(%)"] = float((info["销售费用"]) + info["财务费用"] + info["管理费用"]) / float(
            info["营业收入"])
        dict3["经营安全边际率(%)"] = float(info["四、利润总额"]) / float(info["营业收入"]) / (
                float(info["营业收入"]) - float(info["营业成本"])) / float(info["营业收入"])
        # dict3["EPS=基本每股收益(元)"] =
        ...
        dict4["营业利润增长率(%)"] = (float(info["四、利润总额"]) - float(info0["四、利润总额"])) / float(
            info0["四、利润总额"])
        dict4["净资产增长率(%)"] = (float(info["所有者权益(或股东权益)合计"]) - float(
            info0["所有者权益(或股东权益)合计"])) / float(info0["所有者权益(或股东权益)合计"])
    ...
    def scheduler(self):
        result1 = self.asset_liability_ratio()
        result2 = self.cash_flow_st()
        result3 = self.five_ratio()
        return result1, result2, result3  # , self.last12


if __name__ == '__main__':
    Rp = Report_dealer1(scode='603993', report_date='2022-03-31')
    r = Rp.scheduler()
